...
class ImageClassifier(BaseClassifier):
    ...
    def extract_fused_feat(self, inputs, data_samples):
        x = self.backbone(inputs)

        if self.with_neck:
            x = self.neck(x)

        return self.head.get_fused_feat(x, data_samples)
    ...
    def predict(self,
        ...
        # s_time = time.time()
        # feats = self.extract_feat(inputs)
        # tmp = self.head.predict(feats, data_samples, **kwargs)
        # e_time = time.time()
        # print('predict time:', e_time - s_time)
        # return tmp
